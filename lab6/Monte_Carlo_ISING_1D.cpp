...
fstream infile ( "config.final" , ios::in );
...
// INCLUDE YOUR CODE HERE
    ...
    Heat.open("output.Heatc.0",ios::app);

    stima_c = (blk_av[ic]/blk_norm - (blk_av[iu]/blk_norm)*(blk_av[iu]/blk_norm))*beta*beta/(double)nspin;
    glob_av[ic]  += stima_c;
...
double Error(double sum, double sum2, int iblk)
{
    ...
    else return fabs(sqrt((sum2/(double)iblk - pow(sum/(double)iblk,2))/(double)(iblk-1)));
}
