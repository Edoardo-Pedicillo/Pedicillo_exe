...
Press.open ("output.press",ios::app);
Epot<<walker[iv]/npart<<endl;
Press<<rho * temp + (walker[iw] + ptail * (double)npart) / vol<<endl;
...
//Potential energy per particle
    Epot << setw(wd) << (double)iblk*nstep <<  setw(wd) << stima_pot << setw(wd) << glob_av[iv]/(double)iblk << setw(wd) << err_pot << endl;
//Pressure
    Pres << setw(wd) << (double)iblk*nstep <<  setw(wd) << stima_pres << setw(wd) << glob_av[iw]/(double)iblk << setw(wd) << err_press << endl;
...
//g(r)
    for(int i=igofr;i<=igofr+nbins;i++)
    {
        ...
        Gofr << iblk<<setw(wd)<<r <<setw(wd)<<glob_av[i]/iblk<<endl;

        if(iblk==nblk)
        {

          err_gdir=Error(glob_av[i],glob_av2[i],i);
